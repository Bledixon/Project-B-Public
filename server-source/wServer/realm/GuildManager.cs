    ...
    public class GuildManager : List<Player>
    {
        ...
        public static GuildManager Add(Player player, Guild guildStruct)
        {
            ...
            if (CurrentManagers.ContainsKey(guildStruct.Name))
            {
                ...
                if (ret._guildStructs.ContainsKey(player.AccountId.ToString()))
                    ret._guildStructs[player.AccountId.ToString()] = guildStruct;
                else
                    ret._guildStructs.Add(player.AccountId.ToString(), guildStruct);
        ...
        public Guild this[int accountId] { get { return _guildStructs.ContainsKey(accountId.ToString()) ? _guildStructs[accountId.ToString()] : new Guild { Name = "" }; } }
        ...
        {
            return this.Where(_ => _.AccountId.ToString() == accountId).Count() > 0;
        }

        public void Remove(string accountId)
        {
            _guildStructs.Remove(accountId);
            base.RemoveAt(this.FindIndex(_ => _.AccountId.ToString() == accountId));
        }

        public new void Remove(Player player)
        {
            _guildStructs.Remove(player.AccountId.ToString());
            base.Remove(player);
        }
        ...
        {
            _guildStructs.Add(player.AccountId.ToString(), new Guild
            {
                Name = Name,
                Fame = 0,
                Rank = 0,
            ...
            {
                p.SendInfo(player + " has joined the Guild!");
            }
        }
